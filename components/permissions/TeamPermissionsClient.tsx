          ...
          <div className="mx-auto max-w-[1240px] px-5 py-8 sm:px-8">
            <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <div className="relative w-full max-w-[390px]">
                ...
                <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search people, email or role…" className="w-full rounded-lg border border-cream-2 bg-white py-2.5 pl-10 pr-3 text-sm outline-none transition focus:border-rust" />
              </div>
...
function MemberDetail({ member, roles, currentUserId, onBack, onSaved, onStatus }: {
  ...
  const status = async () => {
    if (!member.active && !confirm(`Reactivate ${member.name}?`)) return;
    if (member.active && !confirm(`Deactivate ${member.name}? They will immediately lose access, but their history will be kept.`)) return;
